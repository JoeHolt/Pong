...
class GameScene: SKScene {
    ...
    private let gridSize: Int       = 30    //Size of game grid
    ...
    override func didMove(to view: SKView) {
        ...
        self.rBumper = GameObject(CGSize(width: 1.0, height: 4.0), initialPosistion: CGPoint(x: gridBounds - 3, y: 0), initialSpeed: CGPoint(x: 0, y: -1))
    ...
    private func updateBumper(_ bumper: GameObject) {
        ...
        //Left user bumper
        if bumper.speed.y != 0 && bumper.posistion.y < gridBound-2 && bumper.posistion.y > -(gridBound-1) {
    ...
    private func updateBall() {
        ...
        if newBallPosistion.y >= gridBound-1 {
            ball.speed.y = -1
        }
    ...
    private func drawSquare(x: Int, y: Int, color: SKColor = SKColor.white) {
        let xDim = (x * gridSize) + gridSize/2
        let yDim = (y * gridSize) + gridSize/2
        let square = SKSpriteNode()
        square.position = CGPoint(x: xDim, y: yDim)
        square.size = CGSize(width: gridSize-2, height: gridSize-2)
        square.color = color
        addChild(square)
    }
